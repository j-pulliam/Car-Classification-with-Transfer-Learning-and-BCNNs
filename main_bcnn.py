...
#Date: 9/6/2020
#Purpose: The purpose of this file is to train a BCNN over the training dataset and
...
if __name__ == '__main__':
    #Get the model types to use in the BCNN, image resize shape, whether to perform fine-tuning or feature extraction, batch size, max epochs to train for, and
    #filename to save to
    parser = argparse.ArgumentParser(description="process the command line arguments")
    parser.add_argument("--model1", type=int, default=1, help='numerical value corresponding to the type of CNN model to use for model #1 in the BCNN')
    parser.add_argument("--model2", type=int, default=2, help='numerical value corresponding to the type of CNN model to use for model #2 in the BCNN')
    parser.add_argument("--resize_shape", type=int, default=224, help='image resize shape')
    parser.add_argument("--feature_extract", action='store_true', help='whether or not to perform feature extraction (false = finetune whole model; true = update final layer only)')
    ...
    #Check the image resize shape as BCNNs are only setup to work for 224x224 and 448x448 images
    checkResizeShape(args.resize_shape)
    ...
    #Build the PyTorch BCNN and criterion
    NN = BCNN(args.model1, args.model2, args.resize_shape, args.feature_extract)
